...
describe 'DynamicRouter', type: :request do
  ...
  context 'GET' do
    ...
    it 'works', focus: true do
      json = {a: 'b'}.to_json
      get  "/#{req.project.name.downcase}" + req.request_path
  ...
  context 'PUT' do
    ...
    it 'works' do
      json = {a: 'b'}.to_json
      put "/#{req.project.name}" + req.request_path
      expect(ActiveSupport::JSON.decode(response.body)).to eq(req.return_json)
      expect(response.status).to eq(Rack::Utils::SYMBOL_TO_STATUS_CODE[req.status_code.to_sym])
    end
  end
  ...
  context 'POST' do
    ...
    it 'works' do
      json = {a: 'b'}.to_json
      post "/#{req.project.name}" + req.request_path
      expect(ActiveSupport::JSON.decode(response.body)).to eq(req.return_json)
      expect(response.status).to eq(Rack::Utils::SYMBOL_TO_STATUS_CODE[req.status_code.to_sym])
    end
  end
  ...
  context 'PATCH' do
    ...
    it 'works' do
      json = {a: 'b'}.to_json
      patch "/#{req.project.name}" + req.request_path
      expect(ActiveSupport::JSON.decode(response.body)).to eq(req.return_json)
      expect(response.status).to eq(Rack::Utils::SYMBOL_TO_STATUS_CODE[req.status_code.to_sym])
    end
  end
  ...
  context 'DELETE' do
    ...
    it 'works' do
      json = {a: 'b'}.to_json
      delete "/#{req.project.name}" + req.request_path
      expect(ActiveSupport::JSON.decode(response.body)).to eq(req.return_json)
      expect(response.status).to eq(Rack::Utils::SYMBOL_TO_STATUS_CODE[req.status_code.to_sym])
    end
  end
